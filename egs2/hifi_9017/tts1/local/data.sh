...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    find ${db_root}/hifi_9017 -follow -name "*.flac" | sort | while read -r filename;do
        id=$(basename ${filename} | sed -e "s/\.[^\.]*$//g")
        echo "${id} ${filename}" >> ${scp}
        echo "${id} hifi_9017" >> ${utt2spk}
    done
    ...
    paste -d " " \
        <(cut -d "|" -f 1 < ${db_root}/hifi_9017/metadata.csv) \
        <(cut -d "|" -f 3 < ${db_root}/hifi_9017/metadata.csv) \
        > ${text}
